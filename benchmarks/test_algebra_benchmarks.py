...
@pytest.mark.benchmark(group='semiring-normalize')
@pytest.mark.parametrize('target', ['none', 'class', 'instance'])
def test_benchmark_semiring_normalize(benchmark, target: str):
    candidate = (
        ax.semiring.TropicalSemiring()
        if target == 'instance'
        else (ax.semiring.TropicalSemiring if target == 'class' else None)
    )

    result = benchmark(ax.semiring.Semiring.normalize, candidate)
    assert result is not None
...
def test_benchmark_galois_matrix_mul(benchmark):
    def gf_matrix(seed: int):
        ...
        return {
            r: {
                c: {e: 1 for e in range(8) if local.random() < 0.5} or {0: 1} for c in range(12) if local.random() < 0.4
            }
            for r in range(12)
        }
