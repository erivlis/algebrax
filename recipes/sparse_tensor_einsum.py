...
2. Polymorphic Tropical Tensor Products (algebrax.semiring.TropicalSemiring):
   Executing `einsum` over `TropicalSemiring` computes min-plus tensor shortest-path
...
from algebrax.tensor import einsum, flatten_tensor, outer_product, tensordot, unflatten_tensor
...
def main() -> None:
    ...
    print('      rank tensor einsum contractions, tropical min-plus tensor products,')
    print('      tensor outer products, and nested dictionary conversions.')

    # --- Step 1: Rank-3 Tensor Contraction via einsum ---
    print('\n[Step 1] Rank-3 Multimodal Tensor Contraction via einsum...')
    print('Explanation: C(i, l) = sum_{j, k} A(i, j, k) * B(j, k, l) contracts rank-3 tensors.')

    # Tensor A: User (i) x Item (j) x Context (k)
    tensor_a = AlgebraicTrie(semiring=StandardSemiring)
    ...
    tensor_b = AlgebraicTrie(semiring=StandardSemiring)
    tensor_b[('Movie_A', 'Home', 'Sci-Fi')] = 0.9
    tensor_b[('Movie_B', 'Work', 'Comedy')] = 0.8
    tensor_b[('Movie_C', 'Work', 'Drama')] = 0.95

    # Contraction over shared (j, k) axes via einsum("ijk,jkl->il")
    fused_tensor = einsum('ijk,jkl->il', tensor_a, tensor_b)
    ...
    print('Explanation: einsum over TropicalSemiring computes min-plus shortest latency.')

    trop_a = AlgebraicTrie(semiring=TropicalSemiring)
    trop_a[('Node_1', 'Node_2', 'Route_A')] = 10.0
    trop_a[('Node_1', 'Node_3', 'Route_B')] = 25.0

    trop_b = AlgebraicTrie(semiring=TropicalSemiring)
    trop_b[('Node_2', 'Route_A', 'Dest_X')] = 15.0
    trop_b[('Node_3', 'Route_B', 'Dest_X')] = 5.0

    # Min-Plus Contraction: C(i, l) = min_{j, k} (A(i, j, k) + B(j, k, l))
    trop_fused = einsum('ijk,jkl->il', trop_a, trop_b, semiring=TropicalSemiring())
    ...
    outer_c = outer_product(vec_a, vec_b)
    contract_c = tensordot(vec_a, vec_b, axes=1)
    ...
    flat_trie = flatten_tensor(nested_dict)
    reconstructed_nested = unflatten_tensor(flat_trie)
