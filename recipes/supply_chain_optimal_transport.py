...
================================================================================
1. Multidimensional Tensor Contraction (algebrax.trie.ax.trie.AlgebraicTrie):
   An ax.trie.AlgebraicTrie acts as a multi-dimensional sparse tensor. Setting 3D paths
   (Warehouse, Region, Season) -> Volume and calling `trie.contract((Warehouse,))`
   sums subtree quantities over the ax.semiring.StandardSemiring to yield regional demand totals.
...
3. Relative Entropy Divergence (algebrax.probability.kl_divergence):
   Kullback-Leibler ax.analysis.divergence D_KL(Demand || Supply) = sum d_i * ln(d_i / s_i)
...
def main() -> None:
    ...
    print('      tensors, compute lattice demand bounds, and audit inventory KL ax.analysis.divergence.')

    # --- Step 1: Multidimensional Tensor Contraction via ax.trie.AlgebraicTrie ---
    print('\n[Step 1] Multidimensional Demand Tensor Contraction (ax.trie.AlgebraicTrie)...')
    print('Explanation: ax.trie.AlgebraicTrie acts as a sparse tensor over ax.semiring.StandardSemiring.')
    ...
    print('Explanation: KL Divergence D_KL(Demand || Supply) measures relative ax.probability.entropy')
